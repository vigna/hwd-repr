...
#ifdef MAX_BATCH_SIZE
		// MAX_BATCH_SIZE artifically reduces the batch size for more fine-grained testing
		if (MAX_BATCH_SIZE < next_batch_size) next_batch_size = MAX_BATCH_SIZE;
#endif
...
#else
	fprintf(stderr, "Allocating memory via calloc()... ");
	cs = calloc(mem, 1);
	if (cs == NULL) {
		fprintf(stderr, "Failed.\n");
		exit(1);
...
#elif defined(DYNMT)
	init_dc(DC_INIT);
	mts = get_mt_parameter(32, MT_BITS);
	sgenrand_mt(0, mts);
#endif
